...
module.exports = class {
  data() {
    ...
    // Solo items de esta área
    const all = props.filter(x => String(x.area || "").toLowerCase() === "juan dolio");
    ...
    return {
      // ✅ Usa el layout con SEO centralizado
...
<div class="filters" id="filtersPanel" style="display:none;">
  <div><label>Precio (USD) — Mín.</label><input type="number" id="minPrice" placeholder="0" min="0"></div>
  <div><label>Precio (USD) — Máx.</label><input type="number" id="maxPrice" placeholder="9999999" min="0"></div>
  <div><label>Sector</label><select id="sectorFilter">${sectorOpts}</select></div>
  <div><label>Tipo</label><select id="typeFilter">${typeOpts}</select></div>
  <div class="btns"><button id="applyFilters">Filtrar</button><button id="clearFilters">Limpiar</button></div>
</div>
