...
module.exports = class {
  data() {
    ...
    const all = props.filter(
      x => String(x.area || "").toLowerCase() === "otro"
    );
  ...
  <nav class="sector-nav" aria-label="Zonas destacadas">
    ...
    const pageHref = (n) => (n === 0 ? "/inmuebles-otro" : `/inmuebles-otro-${n + 1}`);
...
<!-- Paginador estático (sin filtros activos) -->
...
<script>
  (function(){
    ...
    const baseHref = n => (n === 0 ? "/inmuebles-otro" : \`/inmuebles-otro-\${n + 1}\`);
