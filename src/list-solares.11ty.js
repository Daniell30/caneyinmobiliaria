...
module.exports = class {
  data() {
    ...
    // Área: SOLARES
    const all = props.filter(x => String(x.area || "").toLowerCase() === "solares");
    ...
    return {
      // ✅ Layout con SEO centralizado (base.njk + macros/seo.njk)
      layout: "layouts/base.njk",

      // ✅ Metadatos SEO de esta sección
      pageTitle: "Solares en venta | Caney Inmobiliaria",
      pageDesc: `Terrenos y solares en venta en República Dominicana. Filtra por precio, sector y tipo. Fotos, detalles y enlaces a cada propiedad.`,
      // pageImage: "/css/images-caney/GENERAL/og-solares.jpg", // opcional si tienes una imagen OG

      // ✅ CSS del listado (el layout lo incluye si existe pageCss)
      pageCSS: "/css/santodomingoinmuebles.css",

      all, sectors, types,

      // Paginación SSR (primera “página” indexable)
      pagination: { data: "all", size: PER_PAGE, alias: "pageItems" },

      permalink: (data) => {
        const n = data.pagination.pageNumber;
        return n === 0 ? "inmuebles-solares.html"
                       : `inmuebles-solares-${n + 1}.html`;
      }
    };
  }
  ...
  render(data) {
    ...
    const areaSectors = sectorPages(allProps)
      .filter(sp => String(sp.area || "").toLowerCase() === "solares");
  ...
  <nav class="sector-nav" aria-label="Zonas destacadas">
    ...
    const pageHref = (n) => (n === 0 ? "/inmuebles-solares" : `/inmuebles-solares-${n + 1}`);
    ...
    // Dataset completo para filtrar en cliente y paginar el resultado
...
<!-- Paginador estático (para navegación sin filtros) -->
...
<script>
  (function(){
    ...
    const baseHref = n => (n === 0 ? "/inmuebles-solares" : \`/inmuebles-solares-\${n + 1}\`);
