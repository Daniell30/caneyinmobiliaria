...
module.exports = class {
  data() {
    ...
    const all = props.filter(
      x => String(x.area || "").toLowerCase() === "santo domingo"
    );
    ...
    return {
      // ✅ use the shared base layout + SEO macro
      ...
      // Tell the layout which CSS file to include for this page
      ...
      permalink: (data) => {
        ...
        return n === 0 ? "INMUEBLES SANTO DOMINGO.html"
  ...
  render(data) {
    ...
    // Server-rendered cards (for SEO / no-JS)
    ...
    const pageHref = (n) => (n === 0 ? "INMUEBLES SANTO DOMINGO.html" : `INMUEBLES SANTO DOMINGO-${n + 1}.html`);
  ...
  <!-- Page-local styles moved from <head> to here (valid in HTML5) -->
  <style>
  ...
  <!-- Static pager (used when no filters are active) -->
  ...
  <script>
    (function(){
      ...
      const pageHref = (n, qs) => (n === 0 ? "INMUEBLES SANTO DOMINGO.html" : \`INMUEBLES SANTO DOMINGO-\${n + 1}.html\`) + (qs ? ('?' + qs) : '');
      ...
      function renderFiltered(pageIndex){
        ...
        // Build pager that keeps filters in the URL (?min&max&sector&type&p)
        const makeQs = (n) => qsFromInputs(n);
        let html = '';
        if (idx > 0) html += \`<a href="\${pageHref(0, makeQs(idx-1))}" class="view-details" style="margin-right:8px;">← Anterior</a>\`;
        if (idx + 1 < totalPages) html += \`<a href="\${pageHref(0, makeQs(idx+1))}" class="view-details">Ver más</a>\`;
